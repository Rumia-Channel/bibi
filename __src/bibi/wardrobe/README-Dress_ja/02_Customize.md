...
最後までドレス開発が終わったら、開発用コマンドで実行されている処理を止めたうえで、改めて `bun run build` を実行し、サーバ設置用のドレスファイルを生成するのを忘れないようにしてください。
...
※2 - もし `bun start` 後に _dresses.mjs を編集した場合、`bun start` し直すまでその成果は反映されません。
